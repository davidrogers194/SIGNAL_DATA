...
test('atomic import, repeat no-op, rollback, frozen protection and results isolation',async()=>{
 ...
 let fail=false;const env={GITHUB_REPOSITORY_URL:'https://github.com/davidrogers194/SIGNAL_DATA',GITHUB_BRANCH:'main',DB:{prepare(sql){let args=[];return {bind(...a){args=a;return this},async run(){return {meta:{changes:Number(db.prepare(sql).run(...args).changes)}}},async first(){return db.prepare(sql).get(...args)||null},async all(){return {results:db.prepare(sql).all(...args)}}}},async batch(statements){db.exec('BEGIN');try{for(const s of statements){await s.run();if(fail)throw Error('forced rollback')}db.exec('COMMIT')}catch(e){db.exec('ROLLBACK');throw e}}}};
 const docs={research:structuredClone(research),results:structuredClone(results)};const original=globalThis.fetch;globalThis.fetch=async url=>new Response(JSON.stringify(String(url).includes('/commits/')?{sha:'a'.repeat(40)}:{encoding:'base64',content:Buffer.from(JSON.stringify(docs[String(url).includes('research.json')?'research':'results'])).toString('base64')}));
 try{
